...
template.helper('percent', function (data) {
    return data != null ? Math.round(data * 10000) / 100 + "%" : "";
});
...
//保存两位小数
template.helper('toZero', function (num) {
    if (num) {
        num = num.toString();
        var index = num.indexOf(".");
        if (index == -1) return num + ".00";
        else return num.substr(index + 1).length < 2 ? num + "0" : num;
    } else return "0.00"
});
